...
- 움직임은 감쇠 스프링 곡선(`SpringEase`, 450ms)을 따르고, 날아가는 그림은 둥근 모서리와 그림자를 가진 창처럼 그려집니다. 스트립 카드는 다시 만들지 않고 유지되므로 자리가 바뀌면 미끄러지고, 새 카드는 서서히 나타나며, 호버하면 살짝 커집니다. 카드를 중앙에 드롭할 때는 `PlanMerge` 로 미리 계산한 자리까지 그림이 커진 뒤 실제 창이 그 아래에 나타납니다.
