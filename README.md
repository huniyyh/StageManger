...
- 움직임은 감쇠 스프링 곡선(`SpringEase`, 480ms)을 따르고, 날아가는 그림은 둥근 모서리와 그림자를 가진 창처럼 그려집니다. 스트립 카드는 세로 가운데에 모이고(넘치면 위에서부터 스크롤), 다시 만들지 않고 유지되므로 자리가 바뀌면 같은 곡선으로 420ms 동안 미끄러지고, 새 카드는 서서히 나타나며, 호버하면 살짝 커집니다. 나가는 창의 그림은 새 카드가 실제로 놓일 자리로 날아갑니다. 카드를 먼저 만들어 자리를 잡되 그림이 도착할 때까지 보이지 않게 두고, 도착하면 그림이 사라지면서 카드가 나타납니다. 카드를 중앙에 드롭할 때는 `PlanMerge` 로 미리 계산한 자리까지 그림이 커진 뒤 실제 창이 그 아래에 나타납니다. 실제 창은 그림이 화면에서 정말로 제자리에 도착한 뒤에야 복원합니다. 스프링 곡선은 끝에서 목표에 정확히 닿도록 보정되어 있어서, 도착한 그림과 실제 창이 픽셀 단위로 겹칩니다. 애니메이션 시계가 끝나도 마지막 프레임은 렌더링과 합성을 거쳐 늦게 나오고, GPU 가 없는 환경에서는 수백 ms 까지 늦기 때문입니다. 그래서 WPF 가 프레임 두 개를 더 그리고 DWM 이 합성을 마칠 때까지 기다린 뒤 창을 띄우고, 그 창이 합성된 것까지 확인한 다음 180ms 동안 디졸브해서 축소본이 선명한 창으로 초점이 맞듯 넘어갑니다. 디졸브를 짧게 두는 이유는 파킹된 동안 내용이 바뀐 창에서 옛 그림이 비쳐 보이는 시간을 줄이기 위해서입니다.
